...
import { ChevronLeft, RotateCcw, Check, X, Zap, Wand2, RefreshCw, ArrowLeftRight, Timer, UserX, Users } from "lucide-react";
...
const ROLE_SHORT: Record<string, string> = { HANDLER: "H", CUTTER: "C", HYBRID: "Hy" };
...
export function PlayView({
  ...
  const [playerLineOverrides, setPlayerLineOverrides] = useState<Record<string, string[]>>({});
  ...
  return (
    <>
      {step === "outcome" ? (
        ...
        <div className="max-w-lg mx-auto px-4 py-4 space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <button onClick={() => { setSubbingOutId(null); setStep("select"); }} className="flex items-center gap-1 text-sm text-muted-foreground">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-2">vs {game.opponentName}</p>
            </div>
          </div>

          {/* Selected lineup preview */}
          <div className="space-y-1">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground px-1">On field</p>
            <div className="divide-y divide-border rounded-xl border overflow-hidden">
              {selectedPlayers.map((p) => {
                const isSubbingOut = subbingOutId === p.id;
                return (
                  <div key={p.id} className={cn("flex items-center gap-2 px-3 py-2 bg-card transition-colors", isSubbingOut && "bg-amber-50")}>
                    {p.number != null && (
                      <span className="text-xs font-mono text-muted-foreground w-6 shrink-0 text-right">{p.number}</span>
                    )}
                    <span className="flex-1 text-sm font-medium truncate">{p.name}</span>
                    <span className={cn("text-[10px] px-1.5 py-0.5 rounded font-semibold shrink-0", ROLE_COLORS[p.role])}>
                      {ROLE_SHORT[p.role]}
                    </span>
                    ...
                    <button
                      onClick={() => setSubbingOutId(isSubbingOut ? null : p.id)}
                      className={cn(
          ...
          {subbingOutId && (
            <div className="space-y-1">
              ...
              {benchPlayers.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-3">No bench players available</p>
              ) : (
                <div className="divide-y divide-border rounded-xl border overflow-hidden">
                  {benchPlayers.map((p) => (
                    <button
          ...
          <div className="space-y-2">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Who scored?</p>
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                onClick={() => { setScoredByUs(false); setAssistId(null); setGoalId(null); }}
                className={cn(
                  "py-5 rounded-2xl border-2 font-bold text-base transition-all flex items-center justify-center gap-2",
                  scoredByUs === false
          ...
          {scoredByUs === true && (
            <div className="space-y-0.5">
              <div className="flex items-center px-1 pb-1">
                ...
                <span className="w-10 text-center text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Ast</span>
                <span className="w-10 text-center text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Goal</span>
              </div>
              {selectedPlayers.map((p) => {
        ...
        <div className="fixed inset-0 z-[51] flex flex-col bg-background">
          ...
          <div className="max-w-lg mx-auto px-4 pt-3 pb-2 space-y-2">
            ...
            <div className="flex items-center justify-between">
              <Link href={`/tournaments/${game.tournamentId}/games/${game.id}`}>
            ...
            <div className="flex items-center justify-between px-1">
              <span className="text-sm font-medium text-muted-foreground">vs {game.opponentName}</span>
            ...
            {lines.length > 0 && (
              <div className="flex gap-1.5 overflow-x-auto pb-0.5 mt-1">
                ...
                {lines.filter((l) => l.type === "NORMAL").map((line) => (
                  <button
                    key={line.id}
                ...
                {lines.some((l) => l.type === "POWER") && (
                  <span className="self-center text-border text-xs shrink-0 select-none">|</span>
                )}
                {lines.filter((l) => l.type === "POWER").map((line) => (
                  <button
                    key={line.id}
              ...
              return (
                ...
                >
                    ...
                    <div className="flex-1 flex items-center gap-1.5 min-w-0">
                      ...
                      {streak >= 2 && (
                        <span className="flex items-center gap-0.5 text-red-500 shrink-0" title={`${streak} consecutive points`}>
                      ...
                      {isHot && (
                        <span className="text-sm leading-none shrink-0" title="Hot — scored recently">
                          🔥
                        </span>
                      )}
                    </div>
            ...
            <div className="max-w-lg mx-auto px-4 pt-2 pb-3 space-y-2">
              <div className="flex gap-2">
                {lastLineupIds.size === 7 && (
                  <button
                    onClick={() => setSelectedIds(new Set([...lastLineupIds].filter((id) => !injuredIds.has(id))))}
      ...
      <Sheet open={manageOpen} onOpenChange={setManageOpen}>
        <SheetContent side="bottom" className="max-h-[80vh] flex flex-col overflow-hidden">
          <SheetHeader className="shrink-0">
            <SheetTitle>Manage Roster</SheetTitle>
            <SheetDescription>Reassign lines or mark injuries for this game.</SheetDescription>
          </SheetHeader>
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className={cn("text-sm font-medium", injured && "line-through")}>{p.name}</span>
                      <span className={cn("text-[10px] px-1.5 py-0.5 rounded font-semibold", ROLE_COLORS[p.role])}>
                        {ROLE_SHORT[p.role]}
                      </span>
                    </div>
                    {lines.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {lines.map((line) => {
                          ...
                          return (
                            ...
                            >
                              {line.type === "POWER" && <Zap className="h-2.5 w-2.5" />}
                              {line.name}
                            </button>
                          );
                        })}
